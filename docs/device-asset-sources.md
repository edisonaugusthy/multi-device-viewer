...
Browser dimensions remain preview approximations: the address row is 48 logical pixels and the horizontal navigation row, where present, is 40 pixels. A horizontal home indicator occupies a 12-pixel bottom region. Scrolling does not resize or reposition Duo controls. Keyboard space replaces the lower browser area as needed. Controls remain outside the page viewport, including for viewport-fit=cover, to protect fixed page actions. Native Safari may overlay controls or expose different safe-area metrics. Other iPhones retain their existing browser choices.
