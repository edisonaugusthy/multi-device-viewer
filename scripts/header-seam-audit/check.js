async(page)=>{
 ...
 const devices=await page.evaluate(async()=>{const{devices}=await import('/src/domain/device/device-catalog.ts');return devices.filter(d=>d.brand==='Apple'&&['phone','tablet'].includes(d.type)).map(d=>({id:d.id,name:d.name,type:d.type}));});
 const results=[];
