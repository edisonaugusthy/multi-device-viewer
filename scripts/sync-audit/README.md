...
2. Open a dedicated Playwright CLI Chromium or Firefox session at `http://127.0.0.1:5174`. Use an explicit installed browser executable in its CLI config if necessary.
3. Run `python3 scripts/sync-audit/run.py SESSION_NAME chromium` (or `firefox`). It runs four sequential audit scripts and writes both raw output and a combined JSON result to `output/playwright/sync-audit/`. Any failed assertion exits nonzero.
