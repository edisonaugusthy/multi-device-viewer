...
export function DesignReferencePanel(props: DesignReferencePanelProps) {
  ...
  const [referenceZooms, setReferenceZooms] = useState<Record<string, number>>({});
  const fileInput = useRef<HTMLInputElement>(null);
  const referenceImage = references[activeViewportId];
  const referenceZoom = referenceZooms[activeViewportId] ?? 100;
  ...
  return (
    <aside style={{ width }} className={`relative flex shrink-0 flex-col border-r ${dark ? "border-white/10 bg-[#101217] text-white" : "border-slate-200 bg-[#f8fafc] text-slate-900"}`} aria-label="Design reference">
      ...
      {referenceImage && <div className="flex items-center gap-1 px-3 pb-3" aria-label="Design reference zoom">
        <button type="button" aria-label="Zoom design out" onClick={() => setReferenceZooms((current) => ({ ...current, [activeViewportId]: Math.max(25, referenceZoom - 25) }))} className={`grid h-8 w-8 place-items-center rounded-lg border ${dark ? "border-white/10 text-slate-400 hover:bg-white/[0.06] hover:text-white" : "border-slate-200 bg-white text-slate-500 hover:bg-slate-50"}`}><ZoomOut size={14} /></button>
        <span className={`w-12 text-center text-[9px] font-bold ${dark ? "text-slate-400" : "text-slate-500"}`}>{referenceZoom}%</span>
        <button type="button" aria-label="Zoom design in" onClick={() => setReferenceZooms((current) => ({ ...current, [activeViewportId]: Math.min(400, referenceZoom + 25) }))} className={`grid h-8 w-8 place-items-center rounded-lg border ${dark ? "border-white/10 text-slate-400 hover:bg-white/[0.06] hover:text-white" : "border-slate-200 bg-white text-slate-500 hover:bg-slate-50"}`}><ZoomIn size={14} /></button>
        {referenceZoom !== 100 && <button type="button" onClick={() => setReferenceZooms((current) => ({ ...current, [activeViewportId]: 100 }))} className={`ml-1 h-8 rounded-lg px-2 text-[9px] font-bold ${dark ? "text-slate-400 hover:text-white" : "text-slate-500 hover:text-slate-900"}`}>Reset zoom</button>}
      </div>}

      {referenceImage && mode === "overlay" && <div className="flex items-center gap-2 px-3 pb-3">
        <button type="button" aria-pressed={adjustingOverlay} onClick={() => onAdjustingOverlayChange(!adjustingOverlay)} className={`flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-[9px] font-bold ${adjustingOverlay ? "bg-amber-500 text-white" : dark ? "border border-white/10 text-slate-300" : "border border-slate-200 bg-white text-slate-600"}`}>{adjustingOverlay ? <Move size={12} /> : <Lock size={12} />}{adjustingOverlay ? "Adjust overlay" : "Overlay locked"}</button>
        ...
        <p className={`ml-auto text-right text-[9px] ${dark ? "text-slate-500" : "text-slate-400"}`}>{adjustingOverlay ? "Drag · stretch edges or corner" : "Live page is interactive"}</p>
      </div>}

      <div className="min-h-0 flex-1 p-3 pt-0">
        <div onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={handleDrop} className={`grid h-full place-items-center overflow-auto rounded-xl border-2 border-dashed transition ${dragging ? "border-[#0f9f8f] bg-[#0f9f8f]/10" : dark ? "border-white/10 bg-[#07090d]" : "border-slate-200 bg-white"}`}>
          {referenceImage ? <img src={referenceImage} alt="Imported design reference" draggable={false} className="h-auto max-w-none object-contain" style={{ width: `${referenceZoom}%` }} /> : <button type="button" onClick={() => fileInput.current?.click()} className="max-w-72 p-6 text-center"><ClipboardPaste className={`mx-auto ${dark ? "text-slate-700" : "text-slate-300"}`} size={30} /><p className="mt-3 text-xs font-extrabold">Drop, paste, or choose a design</p><p className={`mt-1 text-[10px] leading-4 ${dark ? "text-slate-500" : "text-slate-400"}`}>Use a PNG, JPG, WebP, or SVG exported from Figma or another design tool. It stays on this device.</p></button>}
        </div>
      </div>
    </aside>
  );
