...
export function DesignReferencePanel(props: DesignReferencePanelProps) {
  ...
  return (
    <aside style={{ width }} className={`relative flex shrink-0 flex-col border-r ${dark ? "border-white/10 bg-[#101217] text-white" : "border-slate-200 bg-[#f8fafc] text-slate-900"}`} aria-label={t("designReference")}>
      <div role="separator" aria-label={t("resizeDesignPanel")} aria-orientation="vertical" onPointerDown={startPanelResize} className="group absolute -right-2 inset-y-0 z-50 w-4 cursor-col-resize touch-none"><span className={`absolute inset-y-0 left-1/2 w-px group-hover:bg-[#0f9f8f] ${dark ? "bg-white/10" : "bg-slate-200"}`} /><span className={`absolute left-1/2 top-1/2 grid h-10 w-5 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border shadow-sm group-hover:border-[#0f9f8f] group-hover:bg-[#0f9f8f] group-hover:text-white ${dark ? "border-white/20 bg-[#1a1e27] text-slate-400" : "border-slate-300 bg-white text-slate-500"}`}><GripVertical size={13} /></span></div>
      <header className={`flex h-11 shrink-0 items-center gap-2 border-b px-3 ${dark ? "border-white/10" : "border-slate-200"}`}>
        <div className="min-w-0"><h2 className="text-[11px] font-extrabold">{t("designReference")}</h2><p className={`truncate text-[9px] ${dark ? "text-slate-500" : "text-slate-400"}`}>{t("designReferenceHelp")}</p></div>
        <button type="button" onClick={onClose} aria-label={t("closeDesignReference")} className={`ml-auto grid h-8 w-8 place-items-center rounded-lg ${dark ? "text-slate-400 hover:bg-white/10 hover:text-white" : "text-slate-400 hover:bg-slate-200 hover:text-slate-700"}`}><X size={15} /></button>
      </header>

      <div className="flex gap-1 overflow-x-auto px-3 pt-3" aria-label={t("referenceViewport")}>
      ...
      <div className="flex flex-wrap items-center gap-2 p-3">
        <button type="button" onClick={() => fileInput.current?.click()} className={`flex h-9 items-center gap-2 rounded-lg border px-3 text-[10px] font-bold ${dark ? "border-white/10 hover:bg-white/[0.05]" : "border-slate-200 bg-white hover:bg-slate-50"}`}><ImagePlus size={14} />{referenceImage ? t("replaceDesign") : t("chooseDesignButton")}</button>
      ...
      {referenceImage && <div className="flex items-center gap-2 px-3 pb-3">
        <div className={`flex rounded-lg border p-0.5 ${dark ? "border-white/10" : "border-slate-200 bg-white"}`}>
          <button type="button" onClick={() => onModeChange("side-by-side")} className={`flex h-7 items-center gap-1.5 rounded-md px-2 text-[9px] font-bold ${mode === "side-by-side" ? "bg-[#0f9f8f] text-white" : dark ? "text-slate-400" : "text-slate-500"}`}><Eye size={12} />{t("sideBySide")}</button>
          <button type="button" onClick={() => { onModeChange("overlay"); onAdjustingOverlayChange(true); }} className={`flex h-7 items-center gap-1.5 rounded-md px-2 text-[9px] font-bold ${mode === "overlay" ? "bg-[#0f9f8f] text-white" : dark ? "text-slate-400" : "text-slate-500"}`}><Layers size={12} />{t("overlay")}</button>
        </div>
        {mode === "overlay" && <label className="flex min-w-0 flex-1 items-center gap-2 text-[9px] font-bold"><span>{t("opacity")}</span><input aria-label={t("designOverlayOpacity")} type="range" min="10" max="90" value={opacity} onChange={(event) => onOpacityChange(Number(event.target.value))} className="min-w-20 flex-1 accent-[#0f9f8f]" /><span>{opacity}%</span></label>}
      </div>}

      {referenceImage && <div className="flex items-center gap-1 px-3 pb-3" aria-label={t("designReferenceZoom")}>
        <button type="button" aria-label={t("zoomDesignOut")} onClick={() => scaleReference(-25)} className={`grid h-8 w-8 place-items-center rounded-lg border ${dark ? "border-white/10 text-slate-400 hover:bg-white/[0.06] hover:text-white" : "border-slate-200 bg-white text-slate-500 hover:bg-slate-50"}`}><ZoomOut size={14} /></button>
        <span className={`w-12 text-center text-[9px] font-bold ${dark ? "text-slate-400" : "text-slate-500"}`}>{t("scale")}</span>
        <button type="button" aria-label={t("zoomDesignIn")} onClick={() => scaleReference(25)} className={`grid h-8 w-8 place-items-center rounded-lg border ${dark ? "border-white/10 text-slate-400 hover:bg-white/[0.06] hover:text-white" : "border-slate-200 bg-white text-slate-500 hover:bg-slate-50"}`}><ZoomIn size={14} /></button>
        <button type="button" onClick={() => updateReferencePlacement({ x: 0, y: 0, width: 100, height: 100 })} className={`ml-1 h-8 rounded-lg px-2 text-[9px] font-bold ${dark ? "text-slate-400 hover:text-white" : "text-slate-500 hover:text-slate-900"}`}>{t("reset")}</button>
        <span className={`ml-auto text-[9px] ${dark ? "text-slate-500" : "text-slate-400"}`}>{t("dragResizeImage")}</span>
      </div>}

      {referenceImage && mode === "overlay" && <div className="flex items-center gap-2 px-3 pb-3">
        <button type="button" aria-pressed={adjustingOverlay} onClick={() => onAdjustingOverlayChange(true)} className={`flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-[9px] font-bold ${adjustingOverlay ? "bg-amber-500 text-white" : dark ? "border border-white/10 text-slate-300" : "border border-slate-200 bg-white text-slate-600"}`}><Move size={12} />{t("adjustOverlay")}</button>
        <button type="button" aria-pressed={!adjustingOverlay} onClick={() => onAdjustingOverlayChange(false)} className={`flex h-8 items-center gap-1.5 rounded-lg border px-2.5 text-[9px] font-bold ${!adjustingOverlay ? dark ? "border-teal-500/50 bg-teal-500/15 text-teal-300" : "border-teal-200 bg-teal-50 text-teal-700" : dark ? "border-white/10 text-slate-300" : "border-slate-200 bg-white text-slate-600"}`}><Lock size={12} />{t("lockUsePage")}</button>
        <button type="button" onClick={onResetOverlay} className={`flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-[9px] font-bold ${dark ? "text-slate-400 hover:bg-white/[0.05]" : "text-slate-500 hover:bg-white"}`}><RotateCcw size={12} />{t("resetFit")}</button>
      </div>}
      ...
      <div className="min-h-0 flex-1 p-3 pt-0">
        <div data-reference-canvas onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={handleDrop} className={`relative grid h-full touch-none place-items-center overflow-hidden rounded-xl border-2 border-dashed transition ${dragging ? "border-[#0f9f8f] bg-[#0f9f8f]/10" : dark ? "border-white/10 bg-[#07090d]" : "border-slate-200 bg-white"}`} aria-label={referenceImage ? t("adjustDesignPreview") : undefined}>
          {referenceImage ? <div className="absolute cursor-move select-none border-2 border-sky-400 shadow-[0_0_0_1px_rgba(0,0,0,0.3)]" style={{ left: `${referencePlacement.x}%`, top: `${referencePlacement.y}%`, width: `${referencePlacement.width}%`, height: `${referencePlacement.height}%` }} onPointerDown={(event) => startReferenceAdjustment(event, "move")}><img src={referenceImage} alt={t("importedDesignReference")} draggable={false} className="block h-full w-full" /><button type="button" aria-label={t("resizeReferenceWidth")} onPointerDown={(event) => startReferenceAdjustment(event, "width")} className="absolute -right-2 top-1/2 h-8 w-4 -translate-y-1/2 cursor-ew-resize rounded-full border-2 border-white bg-sky-500 shadow" /><button type="button" aria-label={t("resizeReferenceHeight")} onPointerDown={(event) => startReferenceAdjustment(event, "height")} className="absolute -bottom-2 left-1/2 h-4 w-8 -translate-x-1/2 cursor-ns-resize rounded-full border-2 border-white bg-sky-500 shadow" /><button type="button" aria-label={t("resizeReferenceBoth")} onPointerDown={(event) => startReferenceAdjustment(event, "both")} className="absolute -bottom-2 -right-2 h-5 w-5 cursor-nwse-resize rounded-full border-2 border-white bg-sky-500 shadow" /></div> : <button type="button" onClick={() => fileInput.current?.click()} className="max-w-72 p-6 text-center"><ClipboardPaste className={`mx-auto ${dark ? "text-slate-700" : "text-slate-300"}`} size={30} /><p className="mt-3 text-xs font-extrabold">{t("chooseDesign")}</p><p className={`mt-1 text-[10px] leading-4 ${dark ? "text-slate-500" : "text-slate-400"}`}>{t("chooseDesignHelp")}</p></button>}
        </div>
      </div>
    </aside>
  );
