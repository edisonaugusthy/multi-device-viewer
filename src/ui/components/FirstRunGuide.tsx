      ...
      <div className={`w-full max-w-md rounded-2xl border p-5 shadow-2xl ${dark ? "border-white/10 bg-[#171a21] text-white" : "border-slate-200 bg-white text-slate-900"}`}>
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 id="first-run-title" className="mt-1 text-lg font-extrabold">Review responsive work in three steps</h2>
        ...
        <ol className="mt-5 space-y-3">
          <GuideStep number="1" title="Choose the viewports" text="Start with a quick set or add the exact devices and breakpoints you support." dark={dark} />
          <GuideStep number="2" title="Reproduce the issue" text="Use linked scrolling and interactions to compare the same flow across previews." dark={dark} />
          <GuideStep number="3" title="Generate a fix prompt" text="Click Generate AI fix prompt, describe the problem, and copy the ready-made instructions into your coding tool." dark={dark} />
        </ol>
        ...
        <button type="button" onClick={onClose} className="mt-5 flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-[#0f9f8f] text-xs font-extrabold text-white hover:bg-[#0c8b7e]"><Sparkles size={15} />Start reviewing</button>
      </div>
    </div>
  );
}
