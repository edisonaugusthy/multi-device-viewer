    ...
    <div className="fixed inset-0 z-[75] grid place-items-center bg-black/45 p-4" role="dialog" aria-modal="true" aria-labelledby="review-issue-title">
      <div className={`flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border shadow-2xl ${dark ? "border-white/10 bg-[#171a21] text-white" : "border-slate-200 bg-white text-slate-900"}`}>
        <header className={`flex items-start justify-between border-b p-4 ${dark ? "border-white/10" : "border-slate-100"}`}>
          <div className="min-w-0"><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#0f9f8f]">Generate a coding-agent prompt</p><h2 id="review-issue-title" className="mt-1 text-base font-extrabold">Generate AI fix prompt</h2><p className={`mt-1 text-[11px] ${dark ? "text-slate-400" : "text-slate-500"}`}>Add any details you have. Every field is optional, and the preview includes the active URL, devices, and viewports automatically.</p></div>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 overflow-y-auto p-4 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          <div className="min-w-0 space-y-3">
            ...
            <Field label="Expected behavior"><textarea className={`${inputClass} min-h-16 resize-y`} value={expected} onChange={(event) => setExpected(event.target.value)} placeholder="Navigation should collapse below 768px." /></Field>
            <Field label="Actual behavior"><textarea className={`${inputClass} min-h-16 resize-y`} value={actual} onChange={(event) => setActual(event.target.value)} placeholder="Links wrap over the heading at 390px." /></Field>
            <Field label="Reproduction steps"><textarea className={`${inputClass} min-h-16 resize-y`} value={reproduction} onChange={(event) => setReproduction(event.target.value)} placeholder="Open the page, select the phone viewport, then scroll to the hero." /></Field>
            <Field label="CSS selector"><input className={inputClass} value={selector} onChange={(event) => setSelector(event.target.value)} placeholder="header nav" /></Field>
            <Field label="Constraints and context"><textarea className={`${inputClass} min-h-16 resize-y`} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Keep the desktop navigation unchanged. Reuse the existing menu component." /></Field>
          </div>
        ...
        <footer className={`flex items-center justify-between gap-3 border-t p-4 ${dark ? "border-white/10" : "border-slate-100"}`}>
          <p className={`text-[10px] ${dark ? "text-slate-500" : "text-slate-400"}`}>All fields are optional. Nothing is uploaded; copying only writes the preview to your clipboard.</p>
